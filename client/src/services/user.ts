...
function enableUser(user){
    ...
    return axios.delete(disableResource(user)).then(data=>{
        notification.success(window.W_L.account + userName + window.W_L.activated + "!");
        user.is_disabled= false;
        user.profile_image_url= data.profile_image_url;
        return data;
    }).catch(error=>{
        notification.error(window.W_L.Cannnot_enable_user + getErrorMessage(error));
    })
}

function disableUser(user){
    const userName = sanitize(user.name);
    return axios.post(disableResource(user)).then(data=>{
        notification.warning(window.W_L.account + userName + window.W_L.terminated + "!");
        user.is_disabled= true;
        user.profile_image_url= data.profile_image_url;
        return data;
    }).catch(error=>{
        notification.error(window.W_L.Cannnot_deactivate_user + getErrorMessage(error));
    })
}

function deleteUser(user){
    const userName = sanitize(user.name);
    return axios.delete(`api/users/${user.id}`).then(data=>{
        notification.warning(window.W_L.account + userName + window.W_L.has_been_deleted + "!");
        user.is_disabled= false;
        user.profile_image_url= data.profile_image_url;
        return data;
    }).catch(error=>{
        notification.error(window.W_L.Cannnot_delete_user + getErrorMessage(error));
    })
}
...
function regenerateApiKey(user){
    return axios.post(`api/users/${user.id}/regenerate_api_key`).then(data=>{
...
function sendPasswordReset(user){
    return axios.post(`api/users/${user.id}/password_reset`).then(data=>{
        ...
        notification.error(window.W_L.password_reset_email_has_not_been_sent_successfully, getErrorMessage(error))
    })
}

function resentInvitation(user){
    return axios.post(`api/users/${user.id}/invite`).then(data=>{
        if (clientConfig.mailSettingsMissing){
            notification.warning(window.W_L.the_email_server_is_not_configured);
        ...
        notification.error(window.W_L.user_invitation_email_failed_to_be_sent, getErrorMessage(error))
    })
}
